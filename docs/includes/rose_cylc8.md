...
<!--start:cylc8-workdir-files-->
#### Work directory
_Cylc_ creates a work directory under `$HOME/cycl-run/`, named from your experiment. This directory is organised as follows:
...
- `run1` is the actual work directory for _Cylc_. New installations of the experiment will create a new numbered run directory.
...
The run directory (`run1` in the previous example) contains a copy of the configuration as well as symbolic links to directories under `root_dir` for easy access:
...
│   └── work -> /scratch/tm70/ccc561/cylc-run/another-am3-test/run1/work
```

- `log/job/` &rarr; the directory where all the job log files are stored: the job file itself and the job output and error files
...
In _Cylc8_, running a workflow is a two-step process, first you need to install the work directory and then run the workflow:
