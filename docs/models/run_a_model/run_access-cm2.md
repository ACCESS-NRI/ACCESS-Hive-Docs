...
## (Optional) Set up an ARE VDI Desktop 
...
The {{ model }} configuration can be copied from the MOSRS repository in 2 ways:
...
??? note "Local-only copy"

    ### Local-only copy (recommended) {: #local-copy }
...
??? info "Where is the suite copied to?"
...
??? tip "This section describes how to modify the {{ model }} configuration."
...
??? info "More detailed instructions"
...
??? warning "How to re-open your _Cylc_ GUI"
...
??? danger "Update suites still relying on hh5"
...
??? note "Port suites from accessdev"
